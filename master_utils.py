...
def get_current_sportradar_schedule(api_key, access_level="trial"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            filename = f"sportradar_json_schedule_{year}.json"
...
def get_nflfastr_ids(year):
    ...
    filename = dirname / f"reg_season_play_by_play_{year}.csv"
    ...
    outfile = f"nflfastr_{year}_game_ids.csv"
...
def merge_nflfastr_and_sportradar(year, nflfastr_dirname=None, sportradar_dirname=None):
    ...
    if sportradar_dirname is None:
        sportradar_dirname = Path.cwd() / "DataPack" / "SportRadar" / str(year)
    ...
    nflfastr_csv = nflfastr_dirname / f"nflfastr_{year}_game_ids.csv"
    ...
    same_yr_pbp_path = nflfastr_dirname / f"reg_season_play_by_play_{year}.csv"
